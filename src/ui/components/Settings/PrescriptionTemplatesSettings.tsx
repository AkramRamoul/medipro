        ...
        <div className="space-y-6 m-8 p-6 bg-card text-foreground border border-border rounded-lg shadow-sm text-left">
            <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                    Créez des modèles d'ordonnance pour gagner du temps lors des prescriptions.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4 border p-4 rounded-md bg-muted/30">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="space-y-1">
                                ...
                                <Input size={1} value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="1 bte" />
                            </div>
                            <div className="space-y-1">
                                <Label className="text-xs">Durée</Label>
                                <Input value={duration} onChange={(e) => setDuration(e.target.value)} placeholder="5 jours" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex gap-2">
                                <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Maton-Soir..." />
                    ...
                    <div className="space-y-2">
                        <h5 className="text-xs font-bold uppercase text-muted-foreground">Médicaments dans le modèle :</h5>
                        <div className="max-h-40 overflow-y-auto border rounded bg-background p-2 space-y-1">
                            {selectedMeds.length === 0 ? (
                                <p className="text-xs italic text-muted-foreground">Aucun médicament ajouté.</p>
                            ) : (
                                selectedMeds.map((med, i) => (
                                    <div key={i} className="flex justify-between items-center text-xs p-1 border-b last:border-0">
                                        ...
                                        <Button variant="ghost" size="icon" className="h-4 w-4 text-destructive" onClick={() => removeMedFromTemplate(i)}>
                ...
                <div className="space-y-4">
                    ...
                    {isLoading ? (
                        ...
                        <p className="text-sm text-muted-foreground italic">Aucun modèle configuré.</p>
                    ) : (
