...
const NewPrescriptionForm = ({
  ...
  const handleSuggestionClick = (medication: Medication) => {
    ...
    setInputValue(medication.brandName || medication.name);
              ...
              {suggestions.length > 0 && (
                <div className="absolute z-50 w-full max-h-60 overflow-y-auto bg-popover border border-border rounded-md shadow-lg mt-1 text-left">
                  {suggestions.map((med, index) => (
                    <div
                      key={`${med.name}-${med.brandName}-${index}`}
                      id={`suggestion-${index}`}
                    ...
                    >
                      <div className="font-semibold">{capitalizeMedName(med.brandName || med.name)}</div>
                      {med.brandName && med.brandName.toLowerCase() !== med.name.toLowerCase() && (
                        <div className="text-xs font-medium text-primary/70 mt-0.5">{capitalizeMedName(med.name)}</div>
                      )}
